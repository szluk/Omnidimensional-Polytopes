function [V V2 S S2] = vsomni(ns, kind, D)
...
% OUTPUT:
% V     - volume 
% V2    - volume 2nd branch if present (or VCIB reflection relation) 
% S     - surface
% S2    - surface 2nd branch if present (or SCIB reflection relation) 
%% (c) Szymon Lukaszyk
...
switch kind(1)
  ...
  case('C') % n-cube
    ...
    V2 = D.^ns .* (i.^-ns) .* ( (-ns).^(-ns./2) );                                       % (n-cube volume  does not branch; use VCIB reflection relation instead)
    S  = 2 .* ns .* A.^(ns-1);                                                           % n-cube surface
    S2 = D.^(ns-1) .* (-2) .* i.^(-ns+1) .* (-ns).^((3-ns)./2);                          % (n-cube surface does not branch; use SCIB reflection relation instead)
  case('B')
